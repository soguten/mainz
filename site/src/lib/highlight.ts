export function highlightTypeScriptCodeBlocks(root: ParentNode): void {
    ...
    const blocks = root.querySelectorAll<HTMLElement>("pre code.language-typescript");
    ...
    for (const block of blocks) {
        ...
        if (hljs.highlight) {
            block.innerHTML = hljs.highlight(rawCode, { language: "typescript", ignoreIllegals: true }).value;
