...
Deno.test("site/workshop: should render an editor-like textarea with live preview and line numbers", () => {
  ...
  try {
    ...
    const previewCode = screen.getBySelector<HTMLElement>(
      ".sandbox-editor-preview code",
    );
    const lineNumbers = screen.component.querySelectorAll(
      ".sandbox-editor-line",
    );
    ...
    const updatedPreviewCode = screen.getBySelector<HTMLElement>(
      ".sandbox-editor-preview code",
    );
    const updatedLineNumbers = screen.component.querySelectorAll(
      ".sandbox-editor-line",
    );

    assertEquals(
      updatedPreviewCode.dataset.rawCode,
      'import { Component } from "mainz";\nclass Todo extends Component {}',
    );
