...
const en: SiteDictionary = {
  ...
  nav: {
    ...
    languageMenuLabel: "Language selector",
    home: "Home",
    journey: "Journey",
    concepts: "Concepts",
    checkpoint: "Checkpoint",
  ...
  journey: {
    ...
    stages: [
      ...
      },
      {
        label: "State",
        description:
          "Use initState for initial values and setState for predictable updates.",
      },
      {
        label: "Events",
        description:
          "Connect buttons and inputs with onClick and onInput handlers.",
      },
      {
        label: "Composition",
    ...
    items: [
      {
        ...
        correctIndex: 0,
        success:
          "Correct. initState runs before first render and keeps bootstrap predictable.",
        failure:
          "Not this one yet. Try again focusing on state bootstrap lifecycle.",
      },
      {
        ...
        correctIndex: 0,
        success:
          "Exactly. setState merges partial state and triggers a new render cycle.",
      ...
      },
      {
        ...
        correctIndex: 0,
        success:
          "Great. Composition helps readability, reuse, and long-term evolution.",
  ...
  sandbox: {
    ...
    finalDescription:
      "Great work. You completed all challenges and now have a full component to use.",
    challenges: [
      {
        ...
        hint:
          'You need both: `import { Component } from "mainz"` and `class Todo extends Component`.',
      },
      {
        title: "Challenge 2: Initial state",
        instruction:
          'Inside class `Todo`, add `initState()` returning `{ draft: "", items: [] }`.',
        starter:
          'import { Component } from "mainz";\n\nclass Todo extends Component {\n    // Step 2: add initState\n}\n',
        success: "Nice. Todo now has predictable initial state.",
        hint:
          "Add `initState()` and return an object with `draft` and `items`.",
      },
      {
        title: "Challenge 3: Event + render",
        instruction:
          "Still in `Todo`, add `handleDraftInput`, `addTodo`, and render input + button + list (`items.map(...)`).",
        starter:
          'import { Component } from "mainz";\n\nclass Todo extends Component {\n    protected override initState() {\n        return {\n            draft: "",\n            items: [],\n        };\n    }\n\n    // Step 3: add handlers and render todo UI\n}\n',
        success: "Excellent. Todo behavior and rendering are complete.",
        hint:
          "Use `setState`, bind `onInput` and `onClick`, and render list with `this.state.items.map(...)`.",
      },
    ],
    finalCode:
