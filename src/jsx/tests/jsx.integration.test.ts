...
const fixtures = await import(
  "./jsx.integration.fixture.tsx"
) as typeof import("./jsx.integration.fixture.tsx");
...
Deno.test("jsx/integration: should pass children to class components through props.children", () => {
  ...
  assertEquals(
    screen.getBySelector("[data-role='consumer']").textContent,
    "ok-tail",
  );
  screen.cleanup();
});
...
Deno.test("jsx/integration: should normalize null and boolean children in TSX", () => {
  const screen = renderMainzComponent(
    fixtures.JSXChildrenNormalizationComponent,
  );
...
Deno.test("jsx/integration: should keep nested class component content after parent re-render", () => {
  const screen = renderMainzComponent(
    fixtures.JSXNestedClassRerenderHostComponent,
  );
...
Deno.test("jsx/integration: controlled textarea should keep value/property in sync", () => {
  ...
  assertEquals(
    screen.getBySelector<HTMLTextAreaElement>("textarea").value,
    "mainz",
  );
  assertEquals(
    screen.getBySelector("[data-role='value']").textContent,
    "mainz",
  );
