...
let spaFixturesPromise:
  ...
  | undefined;
let snapshotFixturePromise:
  | Promise<typeof import("./navigation.snapshot.fixture.ts")>
  | undefined;

async function loadSpaFixtures(): Promise<
  typeof import("./navigation.spa.fixture.ts")
> {
...
async function loadSnapshotFixture(): Promise<
  typeof import("./navigation.snapshot.fixture.ts")
> {
...
async function withSuppressedSpaNavigationFailureLogs<T>(
  run: () => Promise<T>,
): Promise<T> {
  ...
  console.error = (...args: Parameters<typeof console.error>) => {
    if (
      ...
      args[0].startsWith("[mainz] SPA navigation failed.")
    ) {
      return;
    }
...
function createValidationPages(): {
  ...
  return {
    ...
    localizedValidationPage:
      LocalizedValidationPage as unknown as SpaPageConstructor,
  };
}
...
Deno.test("navigation/runtime: should render the current SPA route on startup", async () => {
  ...
  const mountedPage = document.querySelector(
    "#app x-mainz-navigation-spa-docs-page",
  );
...
Deno.test("navigation/runtime: should emit navigation-start before navigation-ready with the same sequence", async () => {
  ...
  document.body.innerHTML =
    '<main id="app"></main><a id="docs-link" href="/docs/intro">Docs</a>';
...
Deno.test("navigation/runtime: should render the SPA notFound page for unknown startup routes", async () => {
  ...
  await waitForNavigationReady({
    ...
    navigationType: "initial",
    message:
      "Expected the SPA notFound startup route to emit navigation ready.",
  });
...
Deno.test("navigation/runtime: should redirect anonymous protected routes to the login page", async () => {
  ...
  assertEquals(
    document.querySelector("#app x-mainz-navigation-spa-login-page")
      ?.textContent,
    "Login page",
  );
...
Deno.test("navigation/runtime: should resolve principal before protected page load and expose it in route context", async () => {
  ...
  assertEquals(
    document.querySelector("#app x-mainz-navigation-spa-protected-page")
      ?.textContent,
    "Dashboard page:user-123",
  );
  assertEquals(
    document.querySelector('#app [data-page="dashboard"]')?.getAttribute(
      "data-user-id",
    ),
    "user-123",
  );
  ...
  assertEquals(
    seenContexts[0]?.authorization?.requirement?.authenticated,
    true,
  );
...
Deno.test("navigation/runtime: should surface a forbidden page when an authenticated principal lacks route access", async () => {
  ...
  assertEquals(
    document.querySelector('#app [data-mainz-authorization="forbidden"]')
      ?.textContent,
    "403 Forbidden",
  );
  assertEquals(
    document.querySelector("#app x-mainz-navigation-spa-admin-page"),
    null,
  );
...
Deno.test("navigation/runtime: should emit navigation-error when managed route loading fails", async () => {
  await withSuppressedSpaNavigationFailureLogs(async () => {
    ...
    assertEquals(
      errorDetail.navigationSequence,
      startDetail.navigationSequence,
    );
...
Deno.test("navigation/runtime: startApp should accept a root component shorthand", async () => {
  const { startApp } = await prepareNavigationTest();
  const { Component, CustomElement } = await import(
    "../../components/index.ts"
  );
  ...
  class RootServicesApp extends Component {
    ...
    override render(): HTMLElement {
      ensureMainzCustomElementDefined(
        RootServicesChild as unknown as CustomElementConstructor & {
          getTagName(): string;
        },
      );
      ...
      section.appendChild(
        document.createElement(RootServicesChild.getTagName()),
      );
      return section;
    }
  }
...
Deno.test("navigation/runtime: should strip locale prefixes and notify locale changes", async () => {
  ...
  await waitForNavigationReady({
    ...
    navigationType: "initial",
    message:
      "Expected the localized docs startup route to emit navigation ready.",
  });

  assertEquals(document.documentElement.lang, "pt");
  assertEquals(seenLocales, ["pt"]);
  assertEquals(
    document.querySelector("#app x-mainz-navigation-spa-docs-page")
      ?.textContent,
    "Docs page:intro",
  );
...
Deno.test("navigation/runtime: should stay on the spa root when navigator falls back to the default locale", async () => {
  ...
  await waitForNavigationReady({
    ...
    navigationType: "initial",
    message:
      "Expected the root default locale fallback to finish navigation for en.",
  });
...
Deno.test("navigation/runtime: should apply generated canonical and hreflang links for spa routes", async () => {
  ...
  document.body.innerHTML =
    '<main id="app"></main><a id="go-home" href="/">Home</a>';
  window.history.replaceState(null, "", "/pt/docs/intro");
  ...
  await waitForNavigationReady({
    ...
    navigationType: "initial",
    message:
      "Expected the localized docs route to emit navigation ready before SEO asserts.",
  });
  ...
  const homeReady = waitForNavigationReady({
    ...
    navigationType: "push",
    message:
      "Expected the generated home link to emit navigation ready after click.",
  });
...
Deno.test("navigation/runtime: should resolve locales for prerendered document-first pages", async () => {
  ...
  await waitForNavigationReady({
    ...
    navigationType: "initial",
    message:
      "Expected the prerendered localized home page to emit navigation ready.",
  });
...
Deno.test("navigation/runtime: should reuse route snapshot for document-first bootstrap without rerunning load", async () => {
  ...
  await waitFor(() =>
    document.querySelector(`#app ${SnapshotDocsPage.getTagName()}`)
      ?.textContent ===
      "snapshot:intro"
  );
...
Deno.test("navigation/runtime: should merge dynamic instance head with inherited head()", async () => {
  ...
  assertEquals(
    document.head.querySelector('meta[property="og:type"]')?.getAttribute(
      "content",
    ),
    "article",
  );
...
Deno.test("navigation/runtime: should render the SPA notFound page for unknown internal links", async () => {
  ...
  await waitFor(() =>
    ...
    document.querySelector("#app x-mainz-navigation-spa-not-found-page") !==
      null
  );
...
Deno.test("navigation/runtime: should ignore SPA links outside the configured basePath", async () => {
  ...
  const outsideAppClick = new MouseEvent("click", {
    bubbles: true,
    cancelable: true,
  });
