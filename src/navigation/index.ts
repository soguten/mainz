...
const MAINZ_APP_DEFINITION_KIND: unique symbol = Symbol.for(
  "mainz.appDefinitionKind",
) as typeof MAINZ_APP_DEFINITION_KIND;
...
export type RoutePathResolver = (
  context: { url: URL; basePath: string },
) => string | null;
...
type NavigationLifecycleBaseArgs = Omit<
  NavigationLifecycleEmissionArgs,
  "sequence"
>;
...
export function defineApp(
  ...
  if (isRoutedAppDefinitionShape(app)) {
    ...
    const definedApp = brandDefinedApp(
      app,
      "routed",
...
function resolveGlobalDefinedAppCaptureStack(): Array<
  (app: DefinedApp) => void
> {
...
export function resolveDefinedAppDefinitionsFromModuleExports(
  ...
  for (const candidate of candidates) {
    if (
      (!isDefinedRoutedApp(candidate) && !isDefinedRootApp(candidate)) ||
      seen.has(candidate)
    ) {
      continue;
    }
...
function isRoutedAppDefinitionShape(
  value: unknown,
): value is RoutedAppDefinition {
  ...
  if (
    "notFound" in record && record.notFound !== undefined &&
    !isRoutedPageEntry(record.notFound)
  ) {
    return false;
  }
...
function isRootComponentConstructor(
  value: unknown,
): value is AppRootComponentConstructor {
...
function validateRoutedAppAuthorizationDefinition(
  app: RoutedAppDefinition,
): void {
  ...
  if (!Array.isArray(authorization.policyNames)) {
    throw new Error(
      `App "${app.id}" authorization.policyNames must be an array.`,
    );
  }
...
function validateRoutedAppI18nDefinition(app: RoutedAppDefinition): void {
  ...
  const normalizedLocales = i18n.locales.map((locale, index) => {
    if (typeof locale !== "string") {
      throw new Error(
        `App "${app.id}" i18n.locales[${index}] must be a string.`,
      );
    }
  ...
  if (
    ...
    i18n.defaultLocale.trim().length === 0
  ) {
    throw new Error(
      `App "${app.id}" i18n.defaultLocale must be a string locale tag.`,
    );
  }

  const normalizedDefaultLocale = normalizeLocaleTag(i18n.defaultLocale);
  const localeSet = new Set(
    normalizedLocales.map((locale) => locale.toLowerCase()),
  );
...
function validateImmediatePageLocaleRestrictions(
  app: RoutedAppDefinition,
): void {
  const pageEntries = app.notFound
    ? [...app.pages, app.notFound]
    : [...app.pages];
...
function startSpaNavigation(
  ...
  const handleClick = (event: Event) => {
    ...
    const anchor = findAnchorFromEvent(event);
    const targetUrl = resolveNavigableAnchorUrl(anchor, {
      basePath: normalizedBasePath,
    });
    if (!targetUrl) {
      return;
...
async function renderSpaRoute(args: {
  ...
  const routeMatch = args.routeMatch ??
    findMatchingSpaRoute(args.routes, currentPath);
  if (!routeMatch) {
    return false;
  }
  ...
  const sequence = args.sequence ??
    createNavigationSequenceState(args.nextNavigationSequence);
  emitNavigationStart({
    sequence,
    mount: args.mount,
    mode: args.mode,
  ...
  try {
    ...
    const pageElement =
      args.navigationType === "initial" && isHtmlElement(existingElement)
        ? existingElement as RoutedPageElement
        : args.mount.ownerDocument.createElement(
          pageTagName,
        ) as RoutedPageElement;
...
function normalizeSpaRoute(
  ...
  if (isSpaLazyPageDefinition(entry)) {
    ...
    if (!path) {
      throw new Error(
        "SPA lazy navigation pages must define an explicit route path.",
      );
    }
...
function compareSpaRoutesByPriority(
  a: NormalizedSpaRoute,
  b: NormalizedSpaRoute,
): number {
...
function matchRoutePath(
  ...
  while (
    routeIndex < routeSegments.length && currentIndex < currentSegments.length
  ) {
    const routeSegment = routeSegments[routeIndex];
    const currentSegment = currentSegments[currentIndex];

    if (routeSegment === "*") {
      params["*"] = currentSegments.slice(currentIndex).map(
    ...
    if (routeSegment.startsWith("[...")) {
      params[routeSegment.slice(4, -1)] = currentSegments.slice(currentIndex)
        .map(
          decodeRouteParamSegment,
        ).join("/");
      return params;
    ...
    if (isBracketDynamicSegment(routeSegment)) {
      params[routeSegment.slice(1, -1)] = decodeRouteParamSegment(
        currentSegment,
      );
      routeIndex += 1;
      currentIndex += 1;
  ...
  if (
    routeIndex === routeSegments.length - 1 && routeSegments[routeIndex] === "*"
  ) {
    params["*"] = currentSegments.slice(currentIndex).map(
      decodeRouteParamSegment,
    ).join("/");
  ...
  if (
    routeIndex === routeSegments.length - 1 &&
    routeSegments[routeIndex].startsWith("[...")
  ) {
    params[routeSegments[routeIndex].slice(4, -1)] = currentSegments.slice(
      currentIndex,
    )
...
function resolveSpaMount(mount?: string | Element): HTMLElement {
  ...
  if (!isHtmlElement(resolved)) {
    throw new Error(
      `SPA navigation could not find the mount element "${selector}".`,
    );
  }
...
async function resolveSpaRoutePage(
  route: NormalizedSpaRoute,
): Promise<SpaPageConstructor> {
  if (route.page) {
    return route.page;
  }
  ...
  if (!route.load) {
    throw new Error(
      `SPA route "${route.path}" has no page constructor or lazy loader.`,
    );
  }
...
function resolveSpaPageModule(module: SpaPageModule): SpaPageConstructor {
  ...
  if (
    module && typeof module === "object" && typeof module.default === "function"
  ) {
...
function applySpaRouteContext(
  element: HTMLElement,
  context: SpaNavigationRenderContext,
): void {
  ...
  const rerender =
    (element as HTMLElement & { rerender?: () => void }).rerender;
...
async function resolvePageRouteData(args: {
  ...
  const hasStaticLoad =
    typeof Reflect.get(args.page as object, "load") === "function";
...
function resolveNavigableAnchorUrl(
  ...
  if (
    !isUrlWithinNavigationBasePath(
      resolvedUrl,
      normalizeNavigationBasePath(options.basePath),
    )
  ) {
    return null;
  }
...
function toAppRelativePath(url: URL, basePath: string): string | null {
  ...
  return normalizeRoutePath(
    url.pathname.slice(basePathWithoutTrailingSlash.length),
  ) ?? "/";
}
...
function isUrlWithinNavigationBasePath(url: URL, basePath: string): boolean {
  ...
  const basePathWithoutTrailingSlash = basePath.slice(0, -1);
  return url.pathname === basePathWithoutTrailingSlash ||
...
function resolveSpaRootRedirectLocale(
  ...
  const locale = localeSegment
    ? locales.find((candidate) =>
      toLocalePathSegment(candidate) === localeSegment
    )
    : undefined;
...
function readPreferredNavigationLocales(): readonly string[] {
  ...
  if (Array.isArray(navigator.languages)) {
    preferredLocales.push(
      ...navigator.languages.filter((value): value is string =>
        typeof value === "string"
      ),
    );
  }
...
function stripLocalePrefixFromPath(
  pathname: string,
  locales?: readonly string[],
): string {
  if (!locales?.length) {
  ...
  const isLocaleSegment = locales.some((locale) =>
...
function resolvePageNavigationOptions(
  ...
  const onLocaleChange = options.onLocaleChange ??
    legacySpaOptions?.onLocaleChange;
...
function assertRegisteredNavigationPolicies(
  ...
  const eagerPages = [
    ...
    ...collectImmediateNavigationPages(
      options.notFound ? [options.notFound] : [],
    ),
  ];
...
function abortNavigationSequence(
  ...
  if (
    !mount || !mode || !navigationType || !path || !matchedPath || !url ||
    !basePath
  ) {
    return undefined;
  }
...
function resolveMainzDefaultLocale(): string | undefined {
  ...
  const fromGlobal =
    (globalThis as Record<string, unknown>).__MAINZ_DEFAULT_LOCALE__;
...
function resolveMainzLocalePrefix(): "except-default" | "always" {
  ...
  const fromGlobal =
    (globalThis as Record<string, unknown>).__MAINZ_LOCALE_PREFIX__;
...
function readMainzAppLocales(): readonly string[] {
  ...
  const appLocalesFromGlobal =
    (globalThis as Record<string, unknown>).__MAINZ_APP_LOCALES__;
  if (Array.isArray(appLocalesFromGlobal)) {
    return appLocalesFromGlobal.filter((value): value is string =>
      typeof value === "string"
    );
  }
...
async function bootstrapDocumentNavigation(
  ...
  const currentPath =
    resolveRoutePath(url, basePath, options.resolvePath, options.locales) ??
      "/";
  const routeMatch = findMatchingSpaRoute(routes, currentPath);
...
async function resolveMountedRouteContext(
  ...
  for (const route of routes) {
    ...
    const params = context.routeMatch?.route === route
      ? context.routeMatch.params
      : {};
...
function readInitialRouteSnapshot(): InitialRouteSnapshot | undefined {
  ...
  if (
    !(snapshotScript instanceof Element) || snapshotScript.tagName !== "SCRIPT"
  ) {
    return undefined;
  }
...
function resolveMatchingSnapshot(
  ...
  if (
    snapshot.path !== args.path || snapshot.matchedPath !== args.matchedPath
  ) {
    return undefined;
  }
