...
Deno.test({
    name: "commands/runtime: each started app should dispatch shortcuts within its own app registry",
    async fn() {
        const { fixtures, startApp } = await setupCommandRuntimeTest();
        document.body.innerHTML = `<div id="left"></div><div id="right"></div>`;

        const leftController = startApp(fixtures.LeftCommandApp, { mount: "#left" });
        const rightController = startApp(fixtures.RightCommandApp, { mount: "#right" });
...
Deno.test({
    name: "commands/runtime: command when predicates should gate shortcut execution",
    async fn() {
        const { fixtures, startApp } = await setupCommandRuntimeTest();
...
Deno.test({
    ...
    async fn() {
        ...
        try {
            ...
            assertThrows(() => runCommand("missing.command", { appId: "stable-app" }));
        } finally {
            controller.cleanup();
        }
    },
...
Deno.test({
    name: "commands/runtime: listCommands should enumerate commands for the resolved app",
    async fn() {
        const { fixtures, listCommands, startApp } = await setupCommandRuntimeTest();
        document.body.innerHTML = `<div id="app"></div>`;
...
Deno.test({
    name: "commands/runtime: duplicate command ids should fail within the same app",
    async fn() {
        const { fixtures, startApp } = await setupCommandRuntimeTest();
        document.body.innerHTML = `<div id="app"></div>`;

        assertThrows(() => {
            const controller = startApp(fixtures.DuplicateCommandApp, { mount: "#app" });
            controller.cleanup();
        });
    },
});
