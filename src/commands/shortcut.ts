...
function resolveShortcutPlatform(platform: ShortcutPlatform = "auto"): ResolvedShortcutPlatform {
...
function readNavigatorString(getValue: () => string | undefined): string | undefined {
    try {
        return getValue();
    } catch {
        return undefined;
...
function normalizeModifierToken(
    ...
    const normalized = TOKEN_ALIASES[part.trim().toLocaleLowerCase()] ?? part.trim();
...
export function detectShortcutPlatform(): ResolvedShortcutPlatform {
    ...
    const userAgentDataPlatform = readNavigatorString(() => userAgentData.userAgentData?.platform);
