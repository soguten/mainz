...
class AppCommandRegistry {
    ...
    attachKeyboardBridge(): void {
        ...
        this.keyboardBridge = (event: KeyboardEvent) => {
            const targetRoot = resolveCommandAppRootFromTarget(event.target) ??
                resolveCommandAppRootFromTarget(this.root.ownerDocument.activeElement) ??
        ...
        this.root.ownerDocument.addEventListener("keydown", this.keyboardBridge, true);
    }

    cleanup(): void {
        if (this.keyboardBridge) {
            this.root.ownerDocument.removeEventListener("keydown", this.keyboardBridge, true);
        }
    ...
    dispatchKeyboardEvent(event: KeyboardEvent): boolean {
        ...
        const candidates = Array.from(this.entries.values())
            ...
            .sort((left, right) => right.registrationSequence - left.registrationSequence);
    ...
    private matchesAnyShortcut(entry: CommandEntry, normalizedEventChord: string): boolean {
    ...
    private syncCommands(commands: readonly RegisteredCommand[]): void {
        ...
        for (const command of commands) {
            ...
            nextEntries.set(command.id, {
                ...
                execute: command.execute as (context: CommandExecutionContext) => void | boolean,
                id: command.id,
                ...
                title: command.title,
                when: command.when as ((context: CommandExecutionContext) => boolean) | undefined,
            });
        }
...
function registerRegistryId(appId: string, registry: AppCommandRegistry): void {
    const registries = APP_REGISTRIES_BY_ID.get(appId) ?? new Set<AppCommandRegistry>();
    registries.add(registry);
    APP_REGISTRIES_BY_ID.set(appId, registries);
}

function unregisterRegistryId(appId: string, registry: AppCommandRegistry): void {
...
function resolveRegistryByAppId(appId: string): AppCommandRegistry {
    ...
    if (registries.size > 1) {
        throw new Error(`Mainz command app "${appId}" is ambiguous across active app roots.`);
    }
...
function resolveRegistryForExecution(
    ...
    const eventRoot = resolveCommandAppRootFromTarget(context.triggerEvent?.target);
    if (eventRoot) {
...
function resolveSingleDocumentAppRoot(ownerDocument: Document): HTMLElement | undefined {
