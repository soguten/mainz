...
export interface SsgPreviewServerOptions {
    rootDir: string;
    host?: string;
    port?: number;
}

export function createSsgPreviewHandler(rootDir: string): (request: Request) => Promise<Response> {
...
export function serveSsgPreview(options: SsgPreviewServerOptions): Deno.HttpServer<Deno.NetAddr> {
    const host = options.host ?? "127.0.0.1";
    const port = options.port ?? 4173;
    const handler = createSsgPreviewHandler(options.rootDir);

    console.log(`[mainz] Previewing SSG output from ${resolve(options.rootDir)} on http://${host}:${port}/`);
