...
> {
  ...
  onMissingTranslation?: (
    key: string,
    locale: keyof Dictionaries & string,
  ) => void;
}
...
> {
  ...
  ) as (keyof Dictionaries & string)[];
  if (locales.length === 0) {
    throw new Error(
      "createAppDictionaryI18n requires at least one locale in dictionaries.",
    );
  }

  const initialLocale = options.initialLocale ??
    detectInitialLocale(locales, options.detect);
