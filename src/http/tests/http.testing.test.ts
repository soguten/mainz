...
Deno.test("http/testing: createMockFetch should route by method and pathname params", async () => {
  const mockFetch = createMockFetch((routes) => {
    routes.get(
      "/stories/:slug",
      ({ params }) => jsonResponse({ slug: params.slug }),
    );
  });
...
Deno.test("http/testing: query should expose query params from url-like inputs", () => {
  ...
  assertEquals(
    query(new URL("https://example.test/stories?sort=desc")).get("sort"),
    "desc",
  );
  assertEquals(
    query(new Request("https://example.test/stories?tag=mainz")).get("tag"),
    "mainz",
  );
});
