...
const fixtures = await import(
  "./mainz-testing.isolation.fixture.tsx"
) as typeof import("./mainz-testing.isolation.fixture.tsx");
...
Deno.test("testing helper/isolation: multiple interactive surfaces should coexist in the same test root without cross-talk", () => {
  const selectionScreen = renderMainzComponent(
    fixtures.IsolationSelectionSurface,
  );
  ...
  try {
    ...
    assertEquals(
      selectionScreen.getBySelector("p[data-role='selection-summary']")
        .textContent,
      "1",
    );
    assertEquals(
      draftScreen.getBySelector("p[data-role='draft-summary']").textContent,
      "idle",
    );

    draftScreen.input(
      "textarea[data-role='draft-input']",
