...
export async function instantiateTemplate(
  ...
  const template = options.templateUrl
    ...
    : await loadTemplate(
      resolveRequiredTemplateRoot(options.templateRoot),
      runtime,
    );
  const remoteFiles: LoadedRemoteTemplateFile[] | undefined =
...
function resolveRequiredTemplateRoot(templateRoot: string | undefined): string {
  if (!templateRoot) {
    throw new Error(
      "Template root is required for built-in and local templates.",
    );
  }
