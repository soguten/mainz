...
Deno.test("cli/mainz init: should reject an unknown project template", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-init-template-unknown-",
  });
  ...
  try {
    ...
    assertStringIncludes(
      result.stderr,
      'Project template "missing" was not found.',
    );
    assertStringIncludes(
      result.stderr,
      "Available project templates: empty, starter.",
    );
  } finally {
    await Deno.remove(cwd, { recursive: true });
  }
...
Deno.test("cli/mainz: should print command help for init", async () => {
  ...
  try {
    ...
    assertStringIncludes(
      result.stdout,
      "mainz init [<name>] [--template <name|source>]",
    );
  } finally {
    await Deno.remove(cwd, { recursive: true });
  }
  ...
  try {
    ...
    denoConfig.imports = {
      ...
      "mainz/config": toFileSpecifier(
        resolve(cliTestsRepoRoot, "src", "config", "index.ts"),
      ),
...
Deno.test("cli/mainz app: create should scaffold an app workspace and target", async () => {
  ...
  try {
    ...
    const mainFile = await Deno.readTextFile(
      resolve(cwd, "docs", "src", "main.tsx"),
    );
...
Deno.test("cli/mainz app: create should reject conflicting positional and flag names", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-app-create-name-conflict-",
  });
  ...
  try {
    ...
    assertStringIncludes(
      result.stderr,
      'received conflicting names "site" and "docs"',
    );
  } finally {
    await Deno.remove(cwd, { recursive: true });
  }
...
Deno.test("cli/mainz app: create should reject combining --template and --type", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-app-create-template-type-conflict-",
  });
...
Deno.test("cli/mainz app: create should reject an unknown template", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-app-create-template-unknown-",
  });
  ...
  try {
    ...
    assertStringIncludes(
      result.stderr,
      'App template "missing" was not found.',
    );
    assertStringIncludes(
      result.stderr,
...
Deno.test("cli/mainz app: create should apply template npm dependencies to deno app workspaces", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-app-create-chart-deno-",
  });
  ...
  try {
    ...
    const appFile = await Deno.readTextFile(
      resolve(cwd, "analytics", "src", "app.ts"),
    );
...
Deno.test("cli/mainz app: create should apply template npm dependencies to node app workspaces", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-app-create-chart-node-",
  });
  ...
  try {
    ...
    const packageJson = JSON.parse(
      await Deno.readTextFile(resolve(cwd, "package.json")),
    ) as {
      dependencies?: Record<string, unknown>;
      workspaces?: string[];
    };
...
Deno.test("cli/mainz app: create should reject templates incompatible with the project runtime", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-app-create-template-runtime-",
  });
...
Deno.test("cli/mainz app: create should accept a local app template source", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-app-create-local-template-",
  });
...
Deno.test("cli/mainz app: create should accept a remote app template source", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-app-create-remote-template-",
  });
  const archive = await createTarGz({
    ...
    "remote-template/files/src/app.ts.tpl":
      'export const marker = "remote {{appName}}";\n',
  });
...
Deno.test("cli/mainz app: create --type root should scaffold a root app", async () => {
  ...
  try {
    ...
    const rootFile = await Deno.readTextFile(
...
Deno.test("cli/mainz app: remove should prune deno workspace entries", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-app-remove-deno-workspace-",
  });
...
Deno.test("cli/mainz app: remove should reject conflicting positional and flag targets", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-app-remove-target-conflict-",
  });
...
Deno.test("cli/mainz vite: dematerialize should remove the managed Vite config and restore vite extensions", async () => {
  ...
  try {
    ...
    const materialize = await runMainz(cwd, [
      "vite",
      "materialize",
      "--target",
      "site",
    ...
    const dematerialize = await runMainz(cwd, [
      "vite",
      "dematerialize",
      "--target",
      "site",
...
async function createTarGz(files: Record<string, string>): Promise<Uint8Array> {
  ...
  for (const [path, content] of Object.entries(files)) {
    ...
    chunks.push(
      new Uint8Array(paddedTarSize(contentBytes.length) - contentBytes.length),
    );
  }
