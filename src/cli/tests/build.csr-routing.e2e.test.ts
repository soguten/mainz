...
Deno.test("e2e/csr preview: mpa build should serve localized routes and custom 404 page", async () => {
    ...
    const notFoundResponse = await handler(new Request("http://127.0.0.1:4173/enadasd"));
    ...
    const localizedNotFoundResponse = await handler(new Request("http://127.0.0.1:4173/pt/dfdfhsdfsdf"));
    ...
    await withHappyDom(async () => {
        ...
        await import(`${pathToFileURL(localizedNotFoundScriptPath).href}?e2e=${Date.now()}-csr-mpa-404-pt`);
        await nextTick();
