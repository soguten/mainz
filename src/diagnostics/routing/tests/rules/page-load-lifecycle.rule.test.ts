...
Deno.test("diagnostics/routing/rules: page load lifecycle should error on static load()", () => {
...
Deno.test("diagnostics/routing/rules: page load lifecycle should error when static and instance load() coexist", () => {
  const diagnostics = collectPageLoadLifecycleDiagnostics(
    {
...
Deno.test("diagnostics/routing/rules: page load lifecycle should error on render(data) without load()", () => {
  const diagnostics = collectPageLoadLifecycleDiagnostics(
    {
  ...
  assertEquals(diagnostics, [{
    ...
    severity: "error",
    message:
      'Page "RenderDataPage" declares render(data) but does not declare load(). ' +
...
Deno.test("diagnostics/routing/rules: page load lifecycle should allow render(data: unknown) without explicit Data", () => {
  const diagnostics = collectPageLoadLifecycleDiagnostics(
    {
      file: "/repo/src/pages/RenderData.page.tsx",
      exportName: "RenderDataPage",
...
Deno.test("diagnostics/routing/rules: page load lifecycle should error on typed render(data) without explicit Data", () => {
  const diagnostics = collectPageLoadLifecycleDiagnostics(
    {
      file: "/repo/src/pages/RenderData.page.tsx",
      exportName: "RenderDataPage",
