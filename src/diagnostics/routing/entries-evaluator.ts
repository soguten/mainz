...
export function analyzeEntriesMember(
  ...
  for (const member of node.members) {
    ...
    if (
      ts.isMethodDeclaration(member) && isNamedProperty(member.name, "entries")
    ) {
    ...
    if (
      ...
      isNamedProperty(member.name, "entries")
    ) {
...
function analyzeEntriesExpressionWithVisited(
  ...
  if (ts.isIdentifier(expression)) {
    ...
    if (callableExpression) {
      ...
      return analyzeCallableEntriesExpression(
        callableExpression,
        context,
        visitedCallables,
      );
  ...
  if (ts.isCallExpression(expression)) {
    const callableResult = analyzeCallableEntriesCall(
      expression,
      context,
      visitedCallables,
    );
...
function readEntryParams(
  ...
  const paramsProperty = entry.properties.find((
    property,
  ): property is ts.PropertyAssignment =>
    ts.isPropertyAssignment(property) &&
    isNamedProperty(property.name, "params")
  );

  if (
    !paramsProperty || !ts.isObjectLiteralExpression(paramsProperty.initializer)
  ) {
    return normalized;
  }
...
function readMappedParamsObject(
  ...
  if (ts.isIdentifier(normalizedExpression)) {
    ...
    const sharedParams = context.paramsObjectValues.get(
...
function resolveMappedEntryCallWithVisited(
  ...
  const callee = resolveRouteCallableExpression(
  ...
  const mergedBindings = buildResolvedCallBindings(
    callee,
  ...
  const nestedCallableName = readCallableIdentifier(
    normalizedReturnedExpression.expression,
  );
