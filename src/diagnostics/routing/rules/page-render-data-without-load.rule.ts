...
export const pageRenderDataWithoutLoadDiagnosticCode =
  "page-render-data-without-load" as const;
...
export function collectPageRenderDataWithoutLoadDiagnostics(
  ...
  if (
    !facts?.hasRenderDataParameter || facts.staticMembers.hasInstanceLoadMember
  ) {
    return [];
  }
  ...
  return [{
    ...
    severity: "error",
    message:
      `Page "${page.exportName}" declares render(data) but does not declare load(). ` +
