  ...
  try {
    ...
    assertEquals(
      diagnostics.some((diagnostic) =>
        diagnostic.code === "dynamic-ssg-missing-entries"
      ),
      true,
    );
    assertEquals(
      diagnostics.some((diagnostic) =>
        diagnostic.code === "dynamic-ssg-invalid-entries"
      ),
      true,
    );
    assertEquals(
      diagnostics.some((diagnostic) =>
        diagnostic.code === "page-static-load-unsupported"
      ),
      true,
    );
    assertEquals(
...
Deno.test("diagnostics/command: should report authorization and DI diagnostics from fixture targets", async () => {
  ...
  try {
    ...
    assertEquals(
      diDiagnostics.some((diagnostic) =>
        diagnostic.code === "di-registration-cycle"
      ),
      true,
    );
  } finally {
...
Deno.test("diagnostics/command: should evaluate multi-app targets by app id and allow explicit --app selection", async () => {
  ...
  try {
    ...
    assertEquals(
      diagnostics.some((diagnostic) => diagnostic.appId === "alpha-app"),
      true,
    );
    assertEquals(
      diagnostics.some((diagnostic) =>
...
Deno.test("diagnostics/command: should evaluate multi-root-app targets by app id and allow explicit --app selection", async () => {
  ...
  try {
    ...
    assertEquals(
      diagnostics.some((diagnostic) => diagnostic.appId === "alpha-root-app"),
      true,
    );
    assertEquals(
      diagnostics.some((diagnostic) =>
...
async function collectFixtureDiagnostics(configPath: string, target: string) {
  ...
  const normalizedConfig = normalizeMainzConfig(loadedConfig.config);
  return await collectDiagnosticsForConfig(
    normalizedConfig,
    { target },
    Deno.cwd(),
  );
