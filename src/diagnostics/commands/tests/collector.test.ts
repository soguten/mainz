...
Deno.test("diagnostics/commands: should discover stable app command registrations from app definitions", async () => {
...
Deno.test("diagnostics/commands: should report duplicate stable command ids per app and respect app selection", async () => {
  const fixtureFile = resolve(
    join(
      Deno.cwd(),
      "src/diagnostics/commands/tests/command-diagnostics.fixture.ts",
