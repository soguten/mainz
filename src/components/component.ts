...
export const ComponentElementBase = (globalThis.HTMLElement ??
  DeferredComponentElementBase) as typeof HTMLElement;
...
export function ensureComponentElementBaseHydrated(): void {
  ...
  Object.setPrototypeOf(
    ComponentElementBase.prototype,
    runtimeHTMLElement.prototype,
  );
...
> extends ComponentElementBase {
  ...
  private shouldSuppressUnauthorizedRender(): boolean {
    ...
    const principal = authorizationContext?.principal ??
      createAnonymousPrincipal();
  ...
  private prepareComponentLoad(): void {
    ...
    if (
      loadKey === this.asyncLoadKey && this.componentLoadState.status !== "idle"
    ) {
      return;
    }
    ...
    if (
      ...
      (renderConfig.policy === "placeholder-in-ssg" ||
        renderConfig.policy === "hide-in-ssg")
    ) {
  ...
  private startComponentLoad(loadKey: string): void {
    ...
    Promise.resolve(loadResult)
      .then((data) => {
        if (
          requestId !== this.activeLoadRequestId || controller.signal.aborted
        ) {
          return;
        }
    ...
    return stableSerializeForLoadKey(
      {
        ...
        route: this.toSerializableRouteContext(
  ...
  private resolveServiceContainer(): ServiceContainer | undefined {
    const fromRoute = typeof this.props === "object" && this.props !== null &&
        "route" in this.props
      ? readServiceContainer(
        (this.props as Record<string, unknown>).route as
          | object
          | null
          | undefined,
      )
  ...
  private hasComponentLoad(): boolean {
    return this.participatesInComponentLoad() &&
  ...
  private syncPortalEntries(): void {
    ...
    for (const marker of nextMarkers) {
      ...
      const nextNodes = toPortalRenderedNodes(
        descriptor.children,
        this.ownerDocument,
      );
      ...
      const patchedNodes = this.patchChildNodeList(
        target,
        currentNodes,
        nextNodes,
      );
  ...
  private getPortalRetainedNodes(): Node[] {
    return Array.from(this.portalEntries.values()).flatMap((entry) =>
      entry.nodes
    );
  }
   ...
   */
  abstract render(
    ...args: PublicComponentRenderArgs<Data>
  ): HTMLElement | DocumentFragment;
