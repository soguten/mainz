...
const fixtures = await import(
  "./component.patching.fixture.tsx"
) as typeof import("./component.patching.fixture.tsx");
...
Deno.test("patchChildren: unkeyed reordering should reuse nodes by position", () => {
  ...
  assert(
    ...
    "Expected first position node to be reused",
  );
  assertEquals(firstAfter?.getAttribute("data-item"), "c");
  assert(
    afterA !== beforeA,
    "Expected item 'a' identity to change without keys",
  );
...
Deno.test("listeners: registerEvent + setState should update UI immediately after click", () => {
  const screen = renderMainzComponent(
    fixtures.ReRegisterListenerStateComponent,
  );

  assertEquals(
    screen.getBySelector("p[data-role='info']").textContent,
    "renders=0 clicks=0",
  );
...
Deno.test("patchChildren: counter-like update should preserve static sibling nodes", () => {
  ...
  assert(
    ...
    "Expected title node to preserve identity",
  );
  assert(
    paragraphAfter === paragraphBefore,
    "Expected label node to preserve identity",
  );
  assert(
    buttonAfter === buttonBefore,
    "Expected button node to preserve identity",
  );
  assert(
    paragraphAfter.firstChild === textBefore,
    "Expected label text node to preserve identity",
  );
...
Deno.test("patchChildren: counter-like text update should avoid replaceChildren churn", () => {
  ...
  const root = screen.getBySelector<HTMLElement>(
    "div[data-role='counter-root']",
  );
...
Deno.test("patchChildren: host styles should persist across component rerenders", () => {
  ...
  assert(
    ...
    "Expected injected style node to preserve identity",
  );
  assert(
    rootAfter === rootBefore,
    "Expected styled root node to preserve identity",
  );
...
Deno.test("fragment root: should not duplicate DOM tree", () => {
  ...
  assert(
    screen.getBySelector("h1") === firstTitle,
    "Expected fragment title to preserve identity",
  );
  assert(
  ...
  assertEquals(screen.component.children.length, 2);
  assertEquals(
    screen.getBySelector("p[data-role='count']").textContent,
    "Count: 2",
  );
...
Deno.test("fragment root: keyed list should preserve identity across insert and reorder", () => {
  ...
  screen.component.setState({ items: ["a", "b", "c"] });
  assert(
    screen.getBySelector("li[data-id='c']") === beforeC,
    "Expected keyed fragment node to survive insert",
  );
  ...
  assert(
    screen.getBySelector("li[data-id='a']") === beforeA,
    "Expected keyed fragment node to survive reorder",
  );
  assert(
    screen.getBySelector("li[data-id='c']") === beforeC,
    "Expected keyed fragment node to keep identity",
  );
...
Deno.test("fragment root: nested fragments should patch text and lists without duplication", () => {
  ...
  assert(
    screen.getBySelector("h1") === titleBefore,
    "Expected nested fragment title to preserve identity",
  );
  assert(
  ...
  assert(
    screen.getBySelector("li[data-id='c']") === beforeC,
    "Expected nested keyed list identity to persist",
  );
  assertEquals(screen.component.querySelectorAll("h1").length, 1);
  assertEquals(
    screen.component.querySelectorAll("ul[data-role='list']").length,
    1,
  );
  assertEquals(screen.component.querySelectorAll("li").length, 3);
  assertEquals(
...
Deno.test("listeners: keyed reorder should preserve the correct listener-to-item association", () => {
  ...
  try {
    const buttonCBefore = screen.getBySelector<HTMLButtonElement>(
      "button[data-id='c']",
    );
    buttonCBefore.click();
    assertEquals(
      screen.getBySelector("p[data-role='summary']").textContent,
      "a=0|b=0|c=1",
    );

    screen.component.setState({ items: ["c", "b", "a"] });

    const buttonCAfter = screen.getBySelector<HTMLButtonElement>(
      "button[data-id='c']",
    );
    assert(
      buttonCAfter === buttonCBefore,
      "Expected keyed button 'c' to preserve identity on reorder",
    );
...
Deno.test("listeners: removed keyed node should not keep a stale handler after removal", () => {
  const screen = renderMainzComponent(fixtures.KeyedListListenerComponent);
  const removedButton = screen.getBySelector<HTMLButtonElement>(
    "button[data-id='b']",
  );
...
Deno.test("patchChildren: svg subtree should update when a sibling component swaps icons", () => {
  ...
  try {
    ...
    const svgAfter = screen.getBySelector<SVGElement>(
      "svg[data-icon='success']",
    );
    const firstShapeAfter = svgAfter.firstElementChild;

    assert(
      svgAfter === svgBefore,
      "Expected root svg node to preserve identity",
    );
