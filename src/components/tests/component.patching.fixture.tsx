...
export class UnkeyedListPatchComponent
  extends Component<{}, { items: string[] }> {
...
export class ReRegisterListenerComponent
...
export class ReRegisterListenerStateComponent
  extends Component<{}, { renders: number; clicks: number }> {
  ...
  override render(): HTMLElement {
    ...
    info.textContent =
      `renders=${this.state.renders} clicks=${this.state.clicks}`;
...
export class ControlledInputTypingComponent
  extends Component<{}, { text: string; observed: string }> {
  protected override initState() {
  ...
  override render(): HTMLElement {
    ...
    info.setAttribute("data-role", "info");
    info.textContent =
      `text=${this.state.text} observed=${this.state.observed}`;
...
export class ControlledCheckedComponent
  extends Component<{}, { checked: boolean }> {
  override onMount(): void {
...
export class FragmentKeyedListPatchComponent
  extends Component<{}, { items: string[] }> {
  protected override initState() {
    return { items: ["a", "c"] };
  }
...
export class NestedFragmentComponent
  extends Component<{}, { count: number; items: string[] }> {
  protected override initState() {
  ...
  override render(): HTMLElement {
    return (
      <div>
        ...
        <p data-role="summary">
          {`a=${this.state.clicks.a ?? 0}|b=${this.state.clicks.b ?? 0}|c=${
            this.state.clicks.c ?? 0
          }`}
        </p>
      </div>
    );
