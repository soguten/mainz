...
const fixtures = await import(
  "./component.inline-events.fixture.tsx"
) as typeof import("./component.inline-events.fixture.tsx");
...
Deno.test("inline events: replaced conditional branch should not keep stale listeners on the old node", () => {
  ...
  const oldActionButton = screen.getBySelector<HTMLButtonElement>(
  ...
  try {
    ...
    assertEquals(
      screen.getBySelector("button[data-role='action']").textContent,
      "secondary:0",
    );
