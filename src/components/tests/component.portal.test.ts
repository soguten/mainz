...
Deno.test("portal: should render into the nearest app overlay layer by default", async () => {
  ...
  const rightController = startApp(fixtures.PortalRightApp, {
    mount: "#right",
  });
  ...
  assertEquals(
    document.querySelector(`#left [data-mainz-portal-layer="overlay"]`),
    null,
  );
  assertEquals(
    document.querySelector(`#right [data-mainz-portal-layer="overlay"]`),
    null,
  );
});
...
Deno.test("portal: should render into a document layer only when scope is document", async () => {
  ...
  const appLayer = document.querySelector(
...
Deno.test("portal: should preserve event ownership and clean up conditional portal content", async () => {
  ...
  const action = layer.querySelector<HTMLButtonElement>(
  ...
  const updatedAction = layer.querySelector<HTMLButtonElement>(
    "[data-testid='portal-action']",
  );
