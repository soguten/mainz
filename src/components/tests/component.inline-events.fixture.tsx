...
export class InlineClosureComponent extends Component<{}, { count: number }> {
  ...
  override render(): HTMLElement {
    ...
    return (
      <button
        type="button"
        onClick={() => this.setState({ count: capturedCount + 1 })}
      >
...
export class InlineLabelComponent
  extends Component<{}, { label: string; last: string }> {
  protected override initState() {
  ...
  override render(): HTMLElement {
    ...
    return (
      <div>
        <button
          type="button"
          onClick={() => this.setState({ last: currentLabel })}
        >
          save
        </button>
        <p>{this.state.last}</p>
  ...
  override render(): HTMLElement {
    return (
      <div>
        ...
        {this.state.showPrimary
          ? (
            <button
              ...
              data-role="action"
              onClick={() =>
                this.setState({ primaryClicks: this.state.primaryClicks + 1 })}
            >
          ...
          : (
            <button
              ...
              onClick={() =>
                this.setState({
                  secondaryClicks: this.state.secondaryClicks + 1,
                })}
            >
        ...
        <p data-role="summary">
          {`${this.state.primaryClicks}|${this.state.secondaryClicks}`}
        </p>
      </div>
    );
  }
