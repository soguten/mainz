...
) as typeof import("./component.render-owner.fixture.tsx");
...
Deno.test("render owner: should keep event ownership isolated across separate roots", () => {
  ...
  try {
    ...
    assertEquals(
      screenA.getBySelector("button[data-role='app-a']").textContent,
      "1",
    );
    assertEquals(
      screenB.getBySelector("button[data-role='app-b']").textContent,
      "0",
    );
...
Deno.test("render owner: cleanup of one root should not detach listeners from another root", () => {
  ...
  const buttonA = screenA.getBySelector<HTMLButtonElement>(
    "button[data-role='app-a']",
  );
...
Deno.test("render owner: parent rerender should preserve child listener ownership", () => {
  ...
  try {
    ...
    assertEquals(
      screen.getBySelector("button[data-role='child-action']").textContent,
      "1",
    );

    screen.click("button[data-role='parent-rerender']");
    assertEquals(
      screen.getBySelector("p[data-role='parent-version']").textContent,
      "1",
    );
...
Deno.test("render owner: parent subtree removal should tear down nested subtree listeners", () => {
  const screen = renderMainzComponent(fixtures.NestedOwnerBoundaryComponent);
  const childButton = screen.getBySelector<HTMLButtonElement>(
    "button[data-role='child-action']",
  );
  ...
  try {
    ...
    assertEquals(
      screen.component.querySelector("button[data-role='child-action']"),
      null,
    );
    assertEquals(
      screen.getBySelector("p[data-role='child-removed']").textContent,
      "removed",
    );
...
Deno.test("render owner: should restore owner stack after render throws", () => {
  ...
  const recoveryScreen = renderMainzComponent(
    fixtures.OwnerBoundClickComponent,
  );
  try {
