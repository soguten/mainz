...
export function requirePageRoutePath(
  pageCtor: object,
  errorMessage: string,
): string {
...
export function resolvePageRenderConfig(
  pageCtor: object,
): PageRenderConfig | undefined {
  const routeOwner = pageCtor as {
    [PAGE_RENDER_MODE]?: PageRenderMode | PageRenderConfig;
  };
...
export function resolvePageLocales(
  pageCtor: object,
...
function applyPageRenderMode(pageCtor: object, config: PageRenderConfig): void {
  (pageCtor as { [PAGE_RENDER_MODE]?: PageRenderConfig })[PAGE_RENDER_MODE] =
    config;
}

function applyPageLocales(pageCtor: object, locales: readonly string[]): void {
  (pageCtor as { [PAGE_LOCALES]?: readonly string[] })[PAGE_LOCALES] = locales
    .map((
      locale,
      index,
    ) => {
      try {
        ...
        throw new Error(
          `@Locales() received invalid locale "${locale}" at index ${index}. ${
...
function normalizeStoredPageRenderConfig(
