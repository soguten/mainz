...
Deno.test("build/artifacts: prerender supports app module scripts that already include vite timestamp queries", async () => {
  ...
  try {
    const appDir = await writePrerenderFixtureApp(
      cwd,
      "./src/main.tsx?t=1777814219266",
    );
    const html = await Deno.readTextFile(join(appDir, "index.html"));
    const rendered = await renderSsgAppHtml({
      html,
    ...
    assertStringIncludes(rendered.appHtml, 'data-ssg-card="ready"');
    assert(
      rendered.appHtml.includes("<x-home-page") ||
        rendered.appHtml.includes("<x-home-page-"),
...
Deno.test("build/artifacts: formats client-only resource access errors with SSG guidance", () => {
  const message = formatSsgPrerenderError({
    ...
    error: new ResourceAccessError({
      ...
      resourceName: "current-user",
      message:
        'Resource "current-user" is client-only and cannot execute during SSG.',
    }),
  });
