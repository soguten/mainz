...
Deno.test("build/dev-vite-plugin: should ignore wildcard-accept asset requests and fall through to Vite", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-dev-vite-plugin-asset-",
  });
...
Deno.test("build/dev-vite-plugin: should fall through to the CSR shell for missing dynamic ssg entries with fallback csr", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-dev-vite-plugin-fallback-",
  });
...
Deno.test("build/dev-vite-plugin: should emit opt-in debug timing logs for rendered and cached ssg requests", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-dev-vite-plugin-debug-",
  });
  ...
  try {
    ...
    assertStringIncludes(
      logs[0],
      "[mainz][dev:ssg] rendered /docs -> /docs (en, 200) in ",
    );
    assertStringIncludes(
      logs[1],
      "[mainz][dev:ssg] cache-hit /docs -> /docs (en, 200) in ",
    );
  } finally {
    await Deno.remove(cwd, { recursive: true });
  }
...
Deno.test("build/dev-vite-plugin: should invalidate the cached prerendered html on watcher changes", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-dev-vite-plugin-cache-",
  });
...
Deno.test("build/dev-vite-plugin: should invalidate cached prerendered html during hot updates before the next document request", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-dev-vite-plugin-hot-update-",
  });
...
Deno.test("build/dev-vite-plugin: should ignore changing Vite timestamp queries when loading SSR modules", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-dev-vite-plugin-stable-ssr-",
  });
  ...
  try {
    ...
    invokeConfigureServer(plugin, {
      ...
      async transformIndexHtml(_url: string, html: string) {
        ...
        return html.replace(
          'src="/src/main.js"',
          `src="/src/main.js?t=${transformCallCount}"`,
        );
      },
      async ssrLoadModule(specifier: string) {
        ...
        app.innerHTML = '<section data-page="docs">Stable SSR module</section>';
        const element = app.firstElementChild as
          | (Element & { props?: unknown })
          | null;
        if (!element) {
          throw new Error("Missing rendered page element.");
        }
    ...
    assertStringIncludes(first.body, "Stable SSR module");
    ...
    assertStringIncludes(second.body, "Stable SSR module");

    assertEquals(loadedSpecifiers, [
      "/src/main.js?ssg=mainz-dev",
      "/src/main.js?ssg=mainz-dev",
    ]);
  } finally {
    await Deno.remove(cwd, { recursive: true });
...
Deno.test("build/dev-vite-plugin: should strip changing Vite timestamp queries from the served dev HTML", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-dev-vite-plugin-stable-html-",
  });
  ...
  try {
    ...
    invokeConfigureServer(plugin, {
      ...
      async transformIndexHtml(_url: string, html: string) {
        ...
        return html.replace(
          'src="/src/main.js"',
          `src="/src/main.js?foo=bar&t=${transformCallCount}"`,
        );
      },
      async ssrLoadModule() {
        const app = document.querySelector("#app");
        ...
        app.innerHTML = '<section data-page="docs">Stable HTML</section>';
        const element = app.firstElementChild as
          | (Element & { props?: unknown })
          | null;
        if (!element) {
    ...
    for (let index = 0; index < 2; index += 1) {
      ...
      assertStringIncludes(res.body, 'src="/src/main.js?foo=bar"');
      assertEquals(res.body.includes('src="/src/main.js?foo=bar&t='), false);
    }
  } finally {
    await Deno.remove(cwd, { recursive: true });
  }
...
Deno.test("build/dev-vite-plugin: should refresh title and managed head metadata after watcher invalidation", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-dev-vite-plugin-head-cache-",
  });
...
Deno.test("build/dev-vite-plugin: should surface route-aware SSG prerender errors through Vite", async () => {
  const cwd = await Deno.makeTempDir({
    prefix: "mainz-dev-vite-plugin-error-",
  });
