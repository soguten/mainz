...
export async function loadTargetBuildAppDefinition(
  ...
  }?build-app=${Date.now()}-${Math.random().toString(36).slice(2)}`;
  ...
  try {
    ...
    const candidates = resolveDefinedAppDefinitionsFromModuleExports(
      moduleExports,
    );
    ...
    throw new Error(
      `Could not load app definition from "${resolvedAppFile}": ${
...
export async function loadTargetBuildRoutedAppDefinition(
  ...
  const appDefinition = await loadTargetBuildAppDefinition(
    target,
    cwd,
    runtime,
  );
