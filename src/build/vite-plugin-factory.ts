...
export interface MainzGeneratedVitePluginsOptions {
    ...
    devMiddlewareOptions: Parameters<typeof createMainzDevRouteMiddlewarePlugin>[0];
...
export function createMainzGeneratedVitePlugins(
    ...
    plugins.push(createMainzDevRouteMiddlewarePlugin(options.devMiddlewareOptions));
    return plugins;
}
...
function createMainzTypescriptDecoratorsPlugin(
    ...
    return {
        ...
        transform(code, id) {
            ...
            if (!/\.(?:ts|tsx|mts|cts)$/i.test(normalizedId) || /\.d\.ts$/i.test(normalizedId)) {
                return null;
            }
            if (
