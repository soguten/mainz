...
export function resolveDevRouteRequest(args: {
  ...
  for (const route of args.manifest.routes) {
    ...
    const hasMatchingEntry = routeEntries.some((entry) =>
      entry.locale === (locale ?? route.locales[0]) &&
      recordShallowEqual(entry.params, params)
    );
...
function stripLocalePrefixFromPath(
  ...
  const localeSet = new Set(
    routes.flatMap((route) =>
      route.locales.map((locale) => locale.toLowerCase())
    ),
  );
...
function resolveRequestLocale(args: {
  ...
  if (args.localePrefix === "except-default" && args.defaultLocale) {
    return localeMap.get(args.defaultLocale.toLowerCase()) ??
      args.defaultLocale;
  }
