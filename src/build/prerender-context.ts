...
export async function resolveRoutePrerenderContext(
  ...
  const buildServiceContainer = await resolveTargetBuildServiceContainer(
    job.target,
    cwd,
    runtime,
  );
...
async function resolveSsgRouteEntriesByRouteId(
  ...
  const routeEntriesByRouteId = new Map<
    string,
    readonly ResolvedSsgRouteEntry[]
  >();
  ...
  for (const route of manifest.routes) {
    ...
    for (const locale of route.locales) {
      ...
      for (const [entryIndex, entry] of localizedEntries.entries()) {
        const normalizedParams = normalizeStaticEntryParams(
          entry.params,
          route.path,
        );
        try {
...
async function loadRoutePageConstructor(
  ...
  const moduleUrl = `${
    pathToFileURL(resolve(cwd, route.file)).href
  }?route-page=${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const moduleExports = await runtime.importModule<Record<string, unknown>>(
    moduleUrl,
  );
