...
export function createMainzDevRouteMiddlewarePlugin(
        ...
        configureServer(server) {
            ...
            server.middlewares.use(async (req, res, next) => {
                ...
                try {
                    ...
                    if (resolution.kind === "ssg" && resolution.route && resolution.params) {
                        await respondWithSsgHtml({
                            req,
                            res,
                            server,
    ...
    async function respondWithSsgHtml(args: {
        ...
        args.res.end((args.req.method ?? "GET").toUpperCase() === "HEAD" ? "" : html);
    }
}
...
function formatDevSsgDebugMessage(args: {
    ...
    return `[mainz][dev:ssg] ${args.cacheHit ? "cache-hit" : "rendered"} ${args.requestPath} -> ${args.routePath} (${args.locale}, ${args.statusCode}) in ${args.durationMs}ms`;
}
