...
export async function loadMainzConfig(
  ...
  try {
    ...
    throw new Error(
      `Could not load Mainz config at "${absolutePath}": ${
...
export function normalizeMainzConfig(
  input: MainzConfig,
): NormalizedMainzConfig {
...
function normalizeTargetViteAlias(
  ...
  if (Array.isArray(alias)) {
    const normalized = alias.map((entry) =>
      normalizeTargetViteAliasEntry(targetName, entry)
    );
...
function assertAppAliasCanUseFind(targetName: string, find: string): void {
  if (!find) {
    throw new Error(
      `Target "${targetName}" vite.alias entries must define a non-empty find.`,
    );
  }
...
function normalizeTargetViteDefine(
  ...
  const normalizedEntries = Object.entries(define).map(([key, value]) => {
    ...
    if (!normalizedKey) {
      throw new Error(
        `Target "${targetName}" vite.define entries must use non-empty keys.`,
      );
    }
...
function normalizeMainzRuntime(
  runtime: MainzRuntime | undefined,
): MainzRuntime {
  ...
  throw new Error(
    `Unsupported Mainz runtime "${
      String(runtime)
    }". Use "deno", "node", or "bun".`,
  );
}
...
function assertUniqueTargetNames(targets: NormalizedMainzTarget[]): void {
  ...
  for (const target of targets) {
    ...
    if (seen.has(key)) {
      throw new Error(
        `Duplicate target name "${target.name}" in Mainz config.`,
      );
    }
...
function normalizeSiteUrl(siteUrl: string | undefined): string | undefined {
  ...
  try {
    ...
    throw new Error(
      `Invalid build profile siteUrl "${siteUrl}". Expected an absolute URL.`,
    );
  }
