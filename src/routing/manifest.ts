...
export function buildTargetRouteManifest(
  ...
  const mergedRoutes = aggregateRoutes(
    Array.from(mergedByRouteAndLocale.values()),
  );
...
export function buildSsgOutputEntries(
  ...
  if (notFoundRoutes.length > 1) {
    throw new Error(
      `Target "${manifest.target}" defines multiple notFound routes.`,
    );
  }
  ...
  for (const route of manifest.routes) {
    ...
    for (const locale of route.locales) {
      ...
      const localizedEntries = routeEntries?.filter((entry) =>
        entry.locale === locale
      ) ??
...
export function isDynamicRoutePath(path: string): boolean {
  ...
  return getRouteSegments(normalizedPath).some((segment) =>
    ...
    isBracketDynamicSegment(segment) ||
    segment.startsWith("[...")
  );
}
...
export function materializeRoutePath(
  ...
  const segments = getRouteSegments(normalizedPath).map((segment) => {
    if (segment === "*") {
      ...
      if (!catchAll) {
        throw new Error(
          `Missing "*" param while expanding dynamic route "${path}".`,
        );
      }
...
export function validateRouteEntryParams(
  ...
  const normalizedPath = normalizeRoutePath(path);
  const missingParams = getRequiredRouteParamNames(normalizedPath).filter(
    (paramName) => {
      const value = params[paramName];
...
function shouldPrefixLocaleForRouteLocale(args: {
  ...
  return normalizeLocaleTag(args.locale).toLowerCase() !==
...
function buildFilesystemCandidates(
  input: BuildTargetRouteManifestInput,
): CandidateRoute[] {
  return (input.discoveredPages ?? []).map((page) =>
    buildDiscoveredPageCandidate(page, input)
  );
}
...
function aggregateRoutes(
  expandedRoutes: ExpandedRouteLocale[],
): RouteManifestEntry[] {
  ...
  segments.forEach((segment, index) => {
    ...
    if (segment === "*" || bracketCatchAll) {
      ...
      if (index !== segments.length - 1) {
        throw new Error(
          `Invalid route path "${path}": catch-all segment must be last.`,
        );
      }
...
function assertUniqueParamName(
  paramName: string,
  seen: Set<string>,
  path: string,
): void {
  if (seen.has(paramName)) {
    throw new Error(
      `Invalid route path "${path}": duplicate param name "${paramName}".`,
    );
  }
...
function createRouteHeadLinkKey(link: PageHeadLinkDefinition): string {
  ...
  return `${rel}:${link.href.trim()}:${
    link.hreflang?.trim().toLowerCase() ?? ""
  }`;
}
...
function buildLocalizedRouteHref(args: {
  ...
  const localePrefixPath = shouldPrefixLocale
    ? `/${toLocalePathSegment(args.locale)}`
    : "";
  const href = `${localePrefixPath}${normalizedRoutePath || "/"}`;
  const isLocalizedRootRoute = normalizedRoutePath === "" &&
    localePrefixPath !== "";
  const normalizedHref = isLocalizedRootRoute
    ...
    ? href.slice(0, -1)
    : href;
  const basePathPrefixedHref = prependBasePathToRouteHref(
    normalizedHref,
    args.basePath,
  );
  ...
  const absoluteHref = new URL(basePathPrefixedHref, `${args.siteUrl}/`)
    .toString();
...
function prependBasePathToRouteHref(
  href: string,
  basePath: string | undefined,
): string {
...
function describeRouteSource(
  route: { source: RouteSource; file?: string },
): string {
  if (route.file) {
