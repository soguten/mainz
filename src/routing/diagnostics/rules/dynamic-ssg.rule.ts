...
export function collectDynamicSsgDiagnostics(
    ...
    const missingEntriesDiagnostics = collectDynamicSsgMissingEntriesDiagnostics(page, facts);
    if (missingEntriesDiagnostics.length > 0) {
        return missingEntriesDiagnostics;
    }

    const invalidEntries = collectDynamicSsgInvalidEntriesDiagnostics(page, facts);
