...
async function discoverPagesFromFileWithOptions(
  ...
  const moduleUrl = `${
    pathToFileURL(normalizedFilePath).href
  }?page-discovery=${Date.now()}-${Math.random().toString(36).slice(2)}`;
  ...
  const pages = Object.entries(moduleExports)
    .filter(([exportName]) =>
      !options.exportNames || options.exportNames.includes(exportName)
    )
