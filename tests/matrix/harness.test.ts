...
import type { MatrixArtifact, MatrixBuildRecipe, MatrixFixture } from "./harness.ts";
...
function createInlineStepRunner(): Pick<Deno.TestContext, "step"> {
    const step = (async (...args: unknown[]): Promise<boolean> => {
        const first = args[0];
        const second = args[1];
        const stepDefinition = first as { fn?: (t: Deno.TestContext) => Promise<void> | void };
...
function createInlineArtifact(recipe: MatrixBuildRecipe, cleanup?: () => Promise<void>): MatrixArtifact {
    return {
        recipe,
        context: {
...
Deno.test("matrix/harness: should group cases by fixture, profile, render, and navigation", () => {
    const createCase = (input: Pick<MatrixCase, "name" | "fixture" | "profile">): MatrixCase =>
        matrixTest({
            ...input,
            exercise: {
                render: ["ssg"],
    ...
    const groups = groupMatrixCasesByRecipe(
        [
            ...
            createCase({ name: "profiled", fixture: "RoutedApp", profile: "gh-pages" }),
        ],
        { render: "ssg", navigation: "spa" },
    );
...
Deno.test("matrix/harness: should append recipe diagnostics when fixture build fails", async () => {
    ...
    assertStringIncludes(
        error.message,
        formatMatrixRecipeDiagnostics(combo, [{ recipe, cases: [{
            name: "routing",
            fixture: "RoutedApp",
            exercise: [combo],
            run: async () => {},
...
Deno.test("matrix/harness: should append recipe diagnostics when case execution fails", async () => {
    ...
    assertStringIncludes(error.message, "[matrix] combo: render=csr navigation=spa");
    assertStringIncludes(error.message, "[matrix] artifact: outputDir=dist/test-inline");
