...
import { createTestScreen, type TestScreen } from "../../src/testing/test-screen.ts";
import {
...
import type { MatrixArtifact, MatrixNavigation, MatrixRender } from "./harness.ts";
...
export async function renderBuiltRoute(
    ...
    try {
        ...
        await import(
            `${pathToFileURL(scriptPath).href}?matrix=${Date.now()}-${artifact.recipe.render}-${artifact.recipe.navigation}-${encodeURIComponent(routePath)}`
        );
        await navigationReady;

        const appRoot = document.querySelector("#app");
        assert(appRoot instanceof HTMLElement, `Expected #app container while rendering ${routePath}.`);
...
export async function renderBuiltDocument(args: {
    ...
    try {
        ...
        await import(
            `${pathToFileURL(scriptPath).href}?matrix-document=${Date.now()}-${args.artifact.recipe.render}-${args.artifact.recipe.navigation}-${encodeURIComponent(args.documentHtmlPath)}`
        );
        await navigationReady;

        const appRoot = document.querySelector("#app");
        assert(appRoot instanceof HTMLElement, `Expected #app container while rendering ${args.documentHtmlPath}.`);
