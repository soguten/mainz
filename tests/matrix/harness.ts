...
import { type FixtureId, fixtures, resolveFixtureDefinition } from "./fixtures.ts";
...
const defaultMatrixCombos = testCombinations.map((combo: TestBuildCombination) => ({
    render: combo.mode,
    navigation: combo.navigation,
})) satisfies readonly MatrixCombo[];
const defaultMatrixRenders = ["csr", "ssg"] as const satisfies readonly MatrixRender[];
...
export async function runMatrixCombo(args: {
    ...
    for (const group of groups) {
        const fixtureDefinition = (args.resolveFixtureDefinition ?? resolveFixtureDefinition)(
        ...
        try {
            ...
            const artifactOutputDirs = new Map([[getMatrixRecipeKey(group.recipe), artifact.context.outputDir]]);
...
export function expandMatrixExercise(
    ...
    const renders = exercise.render?.length ? exercise.render : defaultMatrixRenders;
...
function logMatrixDiagnostics(combo: MatrixCombo, groups: readonly MatrixCaseGroup[]): void {
