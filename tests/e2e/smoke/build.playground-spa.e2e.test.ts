...
Deno.test("e2e/playground spa: app without navigation should fall back to a direct-load spa shell", async () => {
  ...
  const playgroundBrowserOutDir = resolve(repoRoot, "dist/playground/browser");
  const rootHtmlPath = resolve(playgroundBrowserOutDir, "index.html");
  ...
  const scriptPath = resolveOutputScriptPath({
    outputDir: playgroundBrowserOutDir,
    scriptSrc,
  });
  ...
  await withHappyDom(async () => {
    ...
    await import(
      `${pathToFileURL(scriptPath).href}?e2e=${Date.now()}-csr-spa-root`
    );
    await nextTick();
...
Deno.test("e2e/playground spa: task contract should route preview through the Mainz CLI", async () => {
  const denoJson = JSON.parse(
    await Deno.readTextFile(resolve(repoRoot, "deno.json")),
  ) as {
    tasks?: Record<string, string>;
  };

  const previewTask = denoJson.tasks?.["preview"];
  const appSpecificPreviewTasks = Object.keys(denoJson.tasks ?? {}).filter((
    taskName,
