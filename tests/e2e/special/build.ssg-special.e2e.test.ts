...
Deno.test(
    "e2e/special localized static output: site app should preserve hydration, preview 404 behavior, and relative SEO",
    async (t) => {
        await buildSiteSsg();
...
Deno.test(
    "e2e/special configured siteUrl: site app should emit absolute SEO links while preserving hydration",
    async (t) => {
        ...
        await t.step("hydration", async () => {
            ...
            assertStringIncludes(ptHtml, "Iniciar trilha guiada");
            const ptScriptSrc = extractModuleScriptSrc(ptHtml);
            assert(ptScriptSrc, "Could not find module script src in publish-profile /pt/ html.");
            ...
            await withHappyDom(async () => {
                ...
                await import(`${pathToFileURL(ptScriptPath).href}?e2e=${Date.now()}-publish-pt`);
                await nextTick();
...
Deno.test(
    "e2e/special configured navigation override: site app should force MPA runtime without enhanced hooks",
    async () => {
...
async function buildSiteSsg(): Promise<void> {
    await runMainzCliCommand(
        ...
        "Failed to build site for SSG special e2e test.",
    );
}
