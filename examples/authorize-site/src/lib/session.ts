...
const sessionPresets = [
  {
    ...
    label: "Mainz Member",
    description:
      "Authenticated org member. Sees account and policy-protected billing routes.",
    defaultPath: "/billing",
    session: {
      userId: "u-member",
  ...
  },
  {
    ...
    label: "Outside Guest",
    description:
      "Authenticated, but not part of the Mainz org claim used by the named policy.",
    defaultPath: "/account",
    session: {
      userId: "u-outsider",
...
export const authorizeSitePolicies: Readonly<
  Record<string, AuthorizationPolicy>
> = {
...
export function describeAuthorizeSitePrincipal(principal: Principal): {
  ...
  const displayName = readStringClaim(principal, "displayName") ??
    principal.id ?? "User";
  const persona = readStringClaim(principal, "persona") ??
...
function readStoredSession(): AuthorizeSiteSessionRecord | null {
  ...
  try {
    ...
    const roles = parsed.roles.filter((role): role is string =>
      typeof role === "string"
    );
    return {
