...
import {
  type AuthorizeSiteNavItem,
  getAuthorizeSiteNavigation,
} from "./navigation.ts";
...
export async function buildAuthorizeSiteShellData(
  args: { principal?: Principal; url: URL },
  ...
  return {
    ...
    resolvedPrincipalJson: JSON.stringify(
      toPrincipalSnapshot(principal),
      null,
      2,
    ),
...
async function buildAccessChecks(
  principal: Principal,
): Promise<AuthorizeSiteAccessCheck[]> {
  ...
  return [
    ...
    },
    {
      ...
      reason: isOrgMember
        ? 'claims.orgId === "mainz"'
        : `claims.orgId === ${
          JSON.stringify(principal.claims.orgId ?? undefined)
        }`,
    },
    {
      label: 'role "owner"',
