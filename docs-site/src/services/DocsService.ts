...
export function buildDocsCatalogFromFiles(
  ...
  const articles = normalizedFiles
    .filter((file): file is DocsNormalizedFile & { kind: "article" } =>
      file.kind === "article"
    )
    .map((file) => ({
      slug: file.slug!,
  ...
  const pages = normalizedFiles
    .filter((file): file is DocsNormalizedFile & { kind: "page" } =>
      file.kind === "page"
    )
    .map((file) => ({
      id: file.id!,
  ...
  const sectionMetadataByKey = new Map<
    string,
    { sectionTitle: string; sectionOrder: number }
  >();
  ...
  for (const article of articles) {
    if (articleBySlug.has(article.slug)) {
      throw new Error(
        `Duplicate docs slug "${article.slug}" found in the docs catalog.`,
      );
    }

    articleBySlug.set(article.slug, article);
    contentPathToSlug.set(
      toDocsContentPathname(article.sourcePath),
      article.slug,
    );
    ...
    if (
      article.group && article.groupTitle && article.groupOrder !== undefined
    ) {
  ...
  for (const page of pages) {
    if (pagesById.has(page.id)) {
      throw new Error(
        `Duplicate docs page id "${page.id}" found in the docs catalog.`,
      );
    }
...
function discoverDocsFiles(rootPath: string): readonly DocsSourceFile[] {
  ...
  const maybeDeno = (globalThis as {
    Deno?: {
      readDirSync(
        path: URL,
      ): Iterable<{ name: string; isDirectory: boolean; isFile: boolean }>;
...
function collectDenoDocsFiles(
  ...
  for (const entry of deno.readDirSync(directoryUrl)) {
    ...
    if (entry.isDirectory) {
      files.push(
        ...collectDenoDocsFiles(deno, childUrl, `${childRelativePath}/`),
      );
      continue;
    }
...
function collectDenoDocsMetaFiles(
  ...
  for (const entry of deno.readDirSync(directoryUrl)) {
    ...
    if (entry.isDirectory) {
      files.push(
        ...collectDenoDocsMetaFiles(deno, childUrl, `${childRelativePath}/`),
      );
      continue;
    }
    ...
    files.push({
      sourcePath: normalizeRootPath(childRelativePath).slice(0, -1),
      attributes: JSON.parse(deno.readTextFileSync(childUrl)) as Record<
        string,
        unknown
      >,
    });
...
function normalizeDocsFile(
  ...
  const currentDirectoryPath = collectDirectoryAncestors(normalizedSourcePath)
    .at(-1);
  ...
  const inheritedAttributes = resolveInheritedDirectoryMeta(
    file.sourcePath,
    directoryMetaByPath,
  );
  ...
  const kind = readOptionalString(attributes, "kind") === "page"
    ? "page"
    : "article";
  ...
  const title = readRequiredString(attributes, "title", file.sourcePath);
  const slug = readOptionalString(attributes, "slug") ??
    deriveSlugFromPath(file.sourcePath);
  const section = readRequiredString(attributes, "section", file.sourcePath);
  const sectionTitle = readRequiredString(
    attributes,
    "sectionTitle",
    file.sourcePath,
  );
  const sectionOrder = readRequiredNumber(
    attributes,
    "sectionOrder",
    file.sourcePath,
  );
...
function resolveInheritedDirectoryMeta(
  ...
  if (articleFileName) {
    ...
    const currentDirectoryMetadata = currentDirectoryPath
      ? directoryMetaByPath.get(currentDirectoryPath)
      : undefined;
    const articleMetadata = currentDirectoryMetadata?.articles?.get(
      articleFileName,
    );
    if (articleMetadata) {
...
function normalizeDocsDirectoryArticles(
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error(
      `Docs meta file "${sourcePath}" must define "articles" as an object.`,
    );
  }
...
function assertDeclaredDocsArticlesExist(
  ...
  for (const file of normalizedFiles) {
    ...
    const directoryPath = collectDirectoryAncestors(normalizedSourcePath).at(
      -1,
    );
    ...
    const discovered = discoveredArticleFilesByDirectory.get(directoryPath) ??
      new Set<string>();
...
function collectDirectoryAncestors(sourcePath: string): readonly string[] {
  ...
  for (let index = 1; index <= segments.length; index += 1) {
    directories.push(
      normalizeDirectoryPath(segments.slice(0, index).join("/")),
    );
  }
...
function buildDocsNavSections(
  ...
  const sections = new Map<
    string,
    {
      ...
      groups: Map<
        string,
        { title: string; order: number; items: DocsNavLeaf[] }
      >;
    }
  >();
...
function resolveDocsMarkdownHrefFromCatalog(
  ...
  if (
    !trimmedHref || trimmedHref.startsWith("#") || hasAbsoluteHref(trimmedHref)
  ) {
    return trimmedHref;
  }
  ...
  const resolvedUrl = new URL(
    trimmedHref,
    new URL(currentPathname, "https://mainz.local"),
  );
...
function parseDocsFrontmatterAttributes(
  block: string,
): Record<string, string | number> {
  const attributes: Record<string, string | number> = {};
  ...
  for (const rawLine of block.split("\n")) {
    ...
    if (separatorIndex === -1) {
      throw new Error(
        `Invalid docs frontmatter line "${line}". Expected "key: value".`,
      );
    }
...
function readRequiredString(
  ...
  if (!value) {
    throw new Error(
      `Docs file "${sourcePath}" is missing required frontmatter field "${key}".`,
    );
  }
