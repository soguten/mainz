...
const fixtures = await import("./DocsHomeContent.fixture.tsx") as typeof import(
...
Deno.test("DocsHomeContent resolves overview cards from the docs service", () => {
    ...
    try {
        assertEquals(view.getBySelector(".docs-title").textContent, "Build documentation that feels like a product");
