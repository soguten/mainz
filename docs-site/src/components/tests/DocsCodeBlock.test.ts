...
Deno.test("DocsCodeBlock rerenders highlighted markup after window load", async () => {
  ...
  try {
    ...
    assertEquals(
      view.getBySelector("code").textContent,
      'console.log("hello");',
    );
    ...
    try {
      ...
      assertStringIncludes(
        view.getBySelector("code").innerHTML,
        "hljs-variable",
      );
    } finally {
      cleanupHighlight();
...
Deno.test("DocsCodeBlock normalizes missing language fences to plaintext", () => {
  ...
  try {
    assertEquals(
      view.getBySelector("code").getAttribute("data-code-language"),
      "plaintext",
    );
    assertEquals(
      view.getBySelector(".docs-code-label").textContent,
      "plaintext",
    );
