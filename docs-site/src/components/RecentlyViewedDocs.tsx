...
export class RecentlyViewedDocs
  extends Component<NoProps, NoState, readonly RecentlyViewedDoc[]> {
  ...
  override placeholder() {
    return (
      <section
        class="docs-recent-pages docs-recent-pages-placeholder"
        aria-live="polite"
      >
...
export function recordRecentlyViewedDoc(entry: RecentlyViewedDoc): void {
  ...
  const next = [
    entry,
    ...readStoredRecentlyViewedDocs().filter((item) =>
...
function readRecentlyViewedDocs(
  currentSlug?: string,
