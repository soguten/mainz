...
Deno.test("docs catalog builder ignores markdown files without docs frontmatter", () => {
  ...
  assertEquals(catalog.navSections.map((section) => section.title), [
    "Getting Started",
  ]);
});
...
Deno.test("docs catalog builder ignores article files not listed in _meta.json articles", () => {
  ...
  assertEquals(catalog.articles.map((article) => article.slug), [
    "testing-overview",
  ]);
});
...
Deno.test("docs helpers resolve markdown links into docs routes", () => {
  ...
  assertEquals(
    docs.resolveMarkdownHref(
      "page-model",
      "../core/render-mode-and-strategy.md#blocking",
    ),
