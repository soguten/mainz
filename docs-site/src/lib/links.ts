...
function readAppLocales(): readonly string[] {
  if (
    typeof __MAINZ_APP_LOCALES__ !== "undefined" &&
    Array.isArray(__MAINZ_APP_LOCALES__)
  ) {
    return __MAINZ_APP_LOCALES__.filter((value): value is string =>
      typeof value === "string"
    );
  }
...
function readDefaultLocale(appLocales: readonly string[]): string | undefined {
  if (
    typeof __MAINZ_DEFAULT_LOCALE__ !== "undefined" &&
    __MAINZ_DEFAULT_LOCALE__?.trim()
  ) {
