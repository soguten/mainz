...
export class ListIdentityScenario extends Component<{}, ListState> {
    ...
    private captureNodes(listSelector: string, attrName: string, ids: string[]) {
        ...
        for (const id of ids) {
            const node = this.querySelector(`${listSelector} li[${attrName}='${id}']`);
            map.set(id, node);
        }
    ...
    private runTransition = (nextItems: string[], label: string) => {
        ...
        const keyedBefore = this.captureNodes("ul[data-list='keyed']", "data-id", survivingIds);
        const unkeyedBefore = this.captureNodes("ul[data-list='unkeyed']", "data-item", survivingIds);

        this.setState({ items: nextItems });

        const keyedAfter = this.captureNodes("ul[data-list='keyed']", "data-id", survivingIds);
        const unkeyedAfter = this.captureNodes("ul[data-list='unkeyed']", "data-item", survivingIds);

        const keyedSummary = this.summarizeIdentity(survivingIds, keyedBefore, keyedAfter);
        const unkeyedSummary = this.summarizeIdentity(survivingIds, unkeyedBefore, unkeyedAfter);

        this.setState({
            report: `${label} | keyed preserved: ${keyedSummary} | unkeyed preserved: ${unkeyedSummary}`,
        });
    };
    ...
    override render() {
        return (
            <section className="card">
                ...
                <p>
                    <code>keyed</code> list uses <code>data-id</code> (acts as key). <code>unkeyed</code> list
                    uses <code>data-item</code> (plain attribute).
                </p>
                ...
                <div className="row">
                    <button type="button" onClick={this.insertInMiddle}>Insert in middle (a,b,c)</button>
                    <button type="button" onClick={this.reorderItems}>Reorder (c,b,a)</button>
