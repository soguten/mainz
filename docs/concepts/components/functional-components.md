...
In Mainz, a functional component is just a function that receives props and
returns nodes.

It does not become a custom element instance, does not own lifecycle, and does
not carry local state.
...
That makes functional components feel much closer to HTML helpers or partial
templates than to stateful application units.
...
Plain HTML elements do not gain local state or lifecycle because you called a
function that returned them. Web Components do, because they are actual element
instances with their own lifecycle.
...
React treats function components as first-class component units. They can own
state through hooks and participate directly in the reconciler as component
boundaries.

Mainz does not do that.

In Mainz, the function is invoked during render and contributes markup to the
current owner. The nearest class component remains the real runtime boundary for
state, lifecycle, and managed DOM event ownership.
