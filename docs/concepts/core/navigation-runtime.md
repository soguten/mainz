...
It means Mainz intentionally stopped a managed navigation because it was superseded, cleaned up, or
...
`entries()` belongs to SSG route expansion during build/prerender, not to client/runtime
navigation. A future build/prerender cancellation model may reuse `AbortSignal`, but that would be
a separate lifecycle from `navigationabort`.
