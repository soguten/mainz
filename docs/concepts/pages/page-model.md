...
defines the stable tag the runtime registers. `@RenderMode(...)` declares the route envelope, while
`static page` stays focused on richer metadata like `head`, `locales`, and `notFound`.
